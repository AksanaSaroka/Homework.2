...
async def get_questions() -> list[Question]:    
    questions = await qsr.get_questions()    
    return questions

@question_router.post('')
async def add_question(question:QuestionAdd = Depends()):
    id = await qsr.add_question(question)
    return {'id':id}

@question_router.get('/{id}')
async def question(id: int) -> list[Question]:
    quiz = await qsr.get_question (id=id)
    if question:
        return question    
